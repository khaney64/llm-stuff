...
- **Timing fallback**: `deriveLlamaTimingMetrics()` normally reads speed straight from the backend's `timings` object. MLX (`mlx_lm.server`) reports `usage` but no `timings` at all, which would blank every throughput number, so the proxy also timestamps the first and last streamed token and can derive `tok/s` from those. The fallback is all-or-nothing: it engages only when the backend supplied no usable timing field whatsoever, so llama.cpp and Ollama keep their backend-reported figures — including their deliberate `null` for an unreliable sample, which the fallback must not backfill.
...
- `power-macos.js` — Apple Silicon power provider; streams `macmon pipe` NDJSON from one long-lived child and returns the latest reading. Reports GPU watts by default; `MACMON_POWER_FIELD` selects `sys`/`cpu+gpu+ane`/etc.
- `proxy.sh` — Shared Linux/macOS proxy launcher. Selects a host profile (power provider, `--gpu-idle`, `--default-ctx`) from `$1`, `$PROXY_HOST_PROFILE`, `hostname -s`, then `uname -s`. `proxy.ps1` is the Windows equivalent.
- `macos-setup.md` — Apple Silicon deployment runbook (Metal build, launchd agents, macmon power, llama-swap).
